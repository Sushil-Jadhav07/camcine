...
export const authService = {
  ...
  async updateProfile(userId, updates) {
    // Only send fields the API accepts for PUT /users/{id}
    const allowedFields = ['first_name', 'last_name', 'phone_number', 'age', 'language_preferences', 'regions', 'role'];
    const apiPayload = {};
    allowedFields.forEach((key) => {
      if (updates[key] !== undefined) apiPayload[key] = updates[key];
    });
    ...
    const updatedUser = normalizeUser({
      ...cachedUser,
      ...(response?.data?.user || response?.user || updates),
      id: userId || cachedUser?.id,
    });
  ...
  async updatePreferences(userId, preferences) {
    ...
    // Map UI preference keys to API fields
    return this.updateProfile(userId, {
      language_preferences: merged.languages || merged.language_preferences || [],
