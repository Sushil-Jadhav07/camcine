...
export function ContentCard({ content, variant = 'default', className = '' }) {
  const { user, isAuthenticated } = useAuthStore();
  const { addToWatchlist, removeFromWatchlist, isInWatchlist } = useWatchlistStore();
  const isSeries = content.type === 'series';
  const inWatchlist = isInWatchlist(content.id);
  const imageSrc = content.poster || content.poster_url || content.thumbnail || content.thumbnail_url || content.backdrop || '/Black Cat.png';
  const cardSizeClass =
    variant === 'large'
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 left-2 md:top-3 md:left-3 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
        <span className={`px-1.5 py-0.5 rounded backdrop-blur-md text-[8px] md:text-[9px] font-black tracking-widest uppercase ${getAccessBadgeClass(content)}`}>
      ...
      <button
        ...
        className={`absolute right-2 top-2 z-20 flex h-8 w-8 items-center justify-center rounded-full border backdrop-blur-md transition ${
          inWatchlist
            ? 'border-[var(--accent)]/60 bg-[var(--accent)] text-white'
            : 'border-white/10 bg-black/45 text-white/80 hover:border-[var(--accent)]/50 hover:text-white'
        }`}
