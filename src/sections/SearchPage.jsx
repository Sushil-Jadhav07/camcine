          ...
          {isLoading ? (
            ...
            <div className="space-y-10 md:space-y-14">
              ...
              {songs.length > 0 && (
                <div>
                  <h3 className="flex items-center gap-2 md:gap-3 text-base md:text-lg font-black text-white mb-4 md:mb-6 uppercase tracking-tight">
                    <Flame className="w-4 h-4 md:w-5 md:h-5" style={{ color: 'var(--accent)' }} /> Songs
                    <span className="text-xs text-white/30 font-normal normal-case tracking-normal">({songs.length})</span>
                  </h3>
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-5">
                    {songs.map((content) => (
                      <div key={content.id} className="hover-lift">
