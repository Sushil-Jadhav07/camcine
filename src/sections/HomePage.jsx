...
const uniqueContent = (items = []) => {
  const seen = new Set();
  return items.filter((item) => {
    const key = item.id || `${item.type || 'content'}:${String(item.title || '').toLowerCase()}`;
              ...
              <div className="max-w-3xl">
                <div className="flex items-center gap-2 md:gap-3 mb-3 md:mb-4 flex-wrap">
                  <div className="flex items-center gap-1.5 text-[var(--accent)] font-bold text-xs md:text-sm">
                    ...
                    {heroContent.tmdbRating || heroContent.rating || 'HD'}
                  </div>
                  <span className="text-white/50 text-xs font-bold uppercase tracking-widest">{heroContent.releaseYear || 'New'}</span>
                  <span className={`px-2 py-0.5 rounded text-[9px] md:text-[10px] font-black uppercase tracking-widest ${getAccessBadgeClass(heroContent)}`}>
                    {getAccessLabel(heroContent)}
                  </span>
                ...
                <p className="text-white/55 text-xs sm:text-sm md:text-base mb-5 md:mb-7 line-clamp-2 md:line-clamp-3 max-w-xl font-medium leading-relaxed">
                  {heroContent.description || 'Now streaming on Camcine'}
                </p>
